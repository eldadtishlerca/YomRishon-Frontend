...
export const BoardNotifications = ({ board, onOpenNortification }) => {
  const timestampToTime = (timeStamps) => {
    ...
    if (currTime.getFullYear() === year) {
      if (currTime.getDate() === day)
        return `${currTime.getHours() - time.getHours()}h ago`
    ...
    <section className="board-notifications-main slideInLeft">
      <div className="board-notifications-header flex">
        <div className="board-notifications-header-buttons flex">
          <span className="menu-button flex">
            <BsThreeDots />
          </span>
          <span
            className="x-button flex"
            onClick={() => {
              onOpenNortification()
            }}
          >
        ...
        <div className="board-notifications-header-navbar-container">
          <nav className="board-notifications-header-navbar flex">
            <div className="a">
              <div className="board-notifications-header-navbar-item flex">
                <span>All</span>
              </div>
            </div>
            <div className="a">
      ...
      <div className="nortification-container flex">
        {defaultNotifications.map((item) => (
          <div
            className={
              item.isRead
                ? 'nortification-item read flex'
                : 'nortification-item unread flex'
            }
            key={item._id}
          >
            <img src={item.imgUrl} alt="" />
            <div className="nortification-item-main flex">
              <div className="nortification-title-container flex">
                <h3>{item.user} </h3>
                <span>{timestampToTime(item.createdAt)}</span>
              </div>
              <div>
                <span className="nortification-title-action">
                  @{item.action}{' '}
                </span>
                <span>you {item.txt}</span>
              </div>
